...
\textbf{GitHub Repository:} \url{https://github.com/gfeliu-rgb/ECGR-5106-HW5}
...
The scratch model uses \texttt{torchvision.models.swin\_t(weights=None)} with its head replaced for 100 classes. Unlike the pretrained runs, all scratch-model parameters are trainable. Because CUDA was not available in the verification session, the measured scratch run uses native 32 by 32 CIFAR-100 inputs, batch size 128, learning rate \texttt{0.001}, and 5 epochs on CPU.

\subsection*{Results}
\begin{center}
\scriptsize
\begin{tabular}{|p{1.0in}|c|c|r|r|r|r|p{1.0in}|}
\hline
Model & Pretrained & Frozen & Trainable Params & Time/Epoch & Val Loss & Test Acc. & Status \\
\hline
Swin-Tiny & Yes & Yes & 76,900 & 851.96 & 2.2969 & 60.56\% & 1 epoch, CUDA \\
\hline
Swin-Small & Yes & Yes & 76,900 & 678.62 & 2.0581 & 64.74\% & 1 epoch, CUDA \\
\hline
...
\subsection*{Analysis}
The completed pretrained runs show a clear transfer-learning advantage after one epoch. Swin-Small reached 64.74\% test accuracy, and Swin-Tiny reached 60.56\%. These accuracies are much higher than the from-scratch ViT and ResNet one-epoch results in Problem 1 because the Swin backbones already contain useful image features from pretraining.
